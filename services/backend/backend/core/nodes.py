...
from backend.tools.chroma_tool import document_search
from backend.utils.utils import get_package_root
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder
from backend.tools.f1_mcp import get_mcp_tools


with open(
    get_package_root(CFG.package_name) / CFG.sys_prompt_path, "r", encoding="utf-8"
) as file:
...
def _llm_with_tools():
    ...
    if LLM_WITH_TOOLS is None:
        ...
        LLM_WITH_TOOLS = ChatPromptTemplate.from_messages(
            ...
            tools=[document_search] + get_mcp_tools(),
        )
...
async def execute_tools(state: AgentState) -> dict:
    ...
    mcp_tool_map = {tool.name: tool for tool in get_mcp_tools()}
